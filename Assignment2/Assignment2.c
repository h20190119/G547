...
#define MB_VID 0x1EC9		//pendrive by Moserbaer
#define MB_PID 0x0001
#define SEA_VID 0x0BC2		//Seagate 
#define SEA_PID 0x2322
...
{	
	...
	//usb_reset_device(device);			//This function can also be used to reset the device
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)  
{
	...
	if(id->idProduct == MB_PID && id->idVendor == MB_VID)
	{
		printk(KERN_INFO "\nMoserbaer Pendrive Plugged in\n");
	}
	
	else	if(id->idProduct == SEA_PID && id->idVendor == SEA_VID)
	{
		printk(KERN_INFO "\nSeagate Plugged in\n");
	}
...
static struct usb_device_id usbdev_table [] = {		//device id table
	{USB_DEVICE(MB_VID, MB_PID)},					//passing device that driver going to support
	{USB_DEVICE(SEA_VID,SEA_PID)},
	{}										//terminating entry
};
...
MODULE_AUTHOR("RISHAV SINGH <h20190119@pilani.bits-pilani.ac.in>");
MODULE_LICENSE("GPL");
